...
impl SerialObserveUi {
    ...
    fn render_serial_form(&mut self, ui: &mut Ui) {
        ...
        ui.label("Port");
        ComboBox::from_id_salt("serial_port_select")
            .width(FIELD_WIDTH)
            .selected_text(if self.config.path.is_empty() {
                "Select a serial port".to_string()
            } else {
                truncate_text(&self.config.path)
            })
            .show_ui(ui, |ui| {
        ...
        ui.add_space(4.0);
        ui.label(RichText::new("Or type a custom path").small());
        ui.text_edit_singleline(&mut self.config.path);
        ...
        ComboBox::from_id_salt("serial_baud_select")
            .width(FIELD_WIDTH)
            .selected_text(truncate_text(&self.config.baud_rate.ui_text()))
            .show_ui(ui, |ui| {
...
fn truncate_text(text: &str) -> String {
    if text.chars().count() > MAX_LABEL_LEN {
        format!(
            "{}...",
            text.chars().take(MAX_LABEL_LEN).collect::<String>()
        )
    } else {
        text.to_string()
    }
...
fn render_named_value_combo<T: Copy + PartialEq>(
    ...
    ComboBox::from_id_salt(id)
        .width(FIELD_WIDTH)
        .selected_text(truncate_text(current.name))
        .show_ui(ui, |ui| {
            for item in values {
